...
static char index[6]={'1','2','3','4','5','6'};
...
printk("-->segment:iovec:%p<--\n",iovec); 
 switch(bio_data_dir(bio)){ //bio_data_dir(biovecio)返回要处理数据的方向  
   ...
   case WRITE:
		...
		//sizew=lenw/4096;
		if(lenw<4096){
			unsigned long addr=volume_addr(1);
			struct page* device_page=pfn_to_page(addr);
			disk_mem=(void *)kmap(device_page)+bio->bi_sector*SECTOR_SIZE;
			...
			for(k=0;k<=sizew;k++){
				unsigned long addr=volume_addr(1)+k;
				struct page* device_page=pfn_to_page(addr);
				disk_mem=(void *)kmap(device_page)+bio->bi_sector*SECTOR_SIZE;
				if(k<sizew){
...
int init_module()
{
	...
	if(res<0){
		...
		int device_num = 3000; ///3000
		createDevice(device_num);
...
/*******/
	...
	struct list_head *p1;
	struct start_list *pv;
      list_for_each(p1, &start_head->list){
            pv = list_entry(p1, struct start_list, list);
		if(pv!=NULL){
			kfree(pv->start);	
		}else{
			printk("**pv is NULL!!**");
...
/*
	struct list_head *p1;
	struct start_list *pv1;
      list_for_each(p1, &start_head->list){
            pv1 = list_entry(p1, struct start_list, list);
		if(pv1!=NULL){
			if(pv1->device_no<10){
				printk("**device_no:%d**\n",pv1->device_no);
				printk("**device_start:%d**\n",pv1->start);
				printk("**device_pfn:%p**\n",virt_to_phys(pv1->start));
			}	
		}else{
			printk("**pv1 is NULL!!**");
		}	
      ...
      list_for_each(p, &pm_volume_arrays->list){
		...
		if(pv->volume_no == volume_no){
			...
			struct list_head *p1;
				...
				list_for_each(p2, &start_head->list){
					sl = list_entry(p2, struct start_list, list);
					if(sl->device_no == pd->device_no){
						kfree(sl->start);
						break;
					}      
				}		
                         //printk("***设备的起始地址是***%lx", (unsigned long)pd->start);
				struct list_head *tmp1;
				tmp1 = p1->prev;
                         list_del(p1);
                         kfree(pd);
                         p1=tmp1;
                               //printk("***设备被释放了吗***%lx", (unsigned long)pd1);
                        }
                   kfree(pv);
			break;
                }
